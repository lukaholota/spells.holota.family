...
**Ціль:** [O6](README.md) · **Статус:** 🔄 в роботі (Крок 1 доведено, Крок 2 SQL підготовлено й
провалідовано на `spells_test`, не застосовано) · **Залежить від:**
...
- `generate-spells.ts` теж **не має фільтра за `ruleset`** і читає всі заклинання. Після сіду
  2024-заклинань статичний `spells.json` і сторінка `/spells` покажуть їх усім. Це другий
  витік, окремий від creator-а, і його треба закрити разом із кроком 1.
- Сідери роблять `setTimeout(100)` між рядками. На 500+ записах це ~50 с на сутність; не
  проблема, але прогін не миттєвий.

Порядок: спершу сідер у тестову БД (`spells_test`), доказ, потім owner-apply у прод.
